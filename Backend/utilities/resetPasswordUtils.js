...
const generateResetToken = (userId) => {
  ...
  return jwt.sign(payload, process.env.RESET_PASSWORD_SECRET, { expiresIn: '1h' });
};
...
const decryptResetToken = (encryptedToken) => {
  try {
    const decoded = jwt.verify(encryptedToken, process.env.RESET_PASSWORD_SECRET);
    return decoded;
  } catch (error) {
...
const saveResetToken = async (userId, token) => {
  try {
    await tokenUtils.saveToken(RESET_TOKEN_PREFIX, userId, token, RESET_TOKEN_EXPIRY);
    return true;
  } catch (error) {
...
const verifyResetToken = async (userId, token) => {
  try {
    const storedToken = await tokenUtils.getToken(RESET_TOKEN_PREFIX, userId);
    ...
    await tokenUtils.deleteToken(RESET_TOKEN_PREFIX, userId);
    return true;
  } catch (error) {
...
const sendResetEmail = async (email, name, encryptedToken) => {
  try {
    ...
    const resetLink = `${process.env.REACT_APP_API_URL}/reset-password?token=${encodeURIComponent(encryptedToken)}`;
